# 重构 — harness-refactor

> **技能标识**: `harness-refactor`
> **使用场景**: "改结构但不改行为"——可读性、职责划分、重复消除、命名、模块边界
> **核心规则（一句话）**: **重构不改变行为——测试先转绿，每步绿了才前进。没有测试保护的代码，先补特征测试（characterization test）再动手。**

## 它做什么？

在**行为不变的约束**下改善代码结构。任何重构都必须有测试保护，小步进行，每步验证。

## 什么时候用？

- `coding-skill` 完成功能后需要清理结构（Red-Green-Refactor 的第三段）
- `expert-reviewer` 标注出"结构问题但行为正确"的问题
- `diagnosing-bugs` 发现代码难读/难改导致排查困难
- 技术债条目（`TECH-DEBT.md` 中的 P2 异味）

## 核心机制

### 前置：行为保护（第一条纪律）

1. 检查现有测试：目标代码是否有测试覆盖？
   - **有** → 先跑绿，作为基线
   - **无/覆盖不足** → 先写**特征测试**（characterization test）：用当前输出/行为固化为断言，跑绿。**禁止**在无保护下直接重构
2. 记录基线：`git stash` 后对比或直接 `git diff`，确认重构前后行为一致

### 小步重构循环

对每个小步：
1. **挑一种重构手法**：提取函数/提取变量/改名/拆分模块/合并重复/替换条件逻辑等（按项目 `编码规范.md`）
2. **改代码**：只做这一步，不混入功能变更
3. **跑测试**：该范围相关的测试必须全绿
4. 测试红 → **立即回退这一步**（`git checkout` 该文件/撤销），不带着红测试继续
5. 绿 → 提交（commit message 含 change ID 与重构手法）

循环直到该范围重构完成。

### 验证行为不变

- 跑目标范围全量相关测试 + 周边测试，确认全绿
- 用 `git diff` 核对：重构不应改变任何对外接口签名、返回语义、日志/错误信息（除非显式约定）
- 需要时跑 `/harness-quality` 的 flow→test 对账，确认业务流仍被覆盖

## 禁止事项

- ❌ 重构与功能变更混在一次提交里（无法区分行为变化来源）
- ❌ 无测试保护直接大改（"反正我看懂了"）
- ❌ 顺手"优化"无关代码（scope creep）
- ❌ 一口气重构大范围后一次性验证（失败难定位）
- ❌ 为了重构引入未经评审的依赖或模式

## 为什么需要它？

| 问题 | 解法 |
|------|------|
| 代码能跑但结构烂，越改越难 | 行为不变前提下系统性改善结构 |
| 重构把行为改了没人发现 | 测试基线 + git diff 核对 |
| 无测试保护直接大改 | 先补特征测试再动手 |
| 重构失败难以定位 | 小步 + 每步绿了才前进 + 红了立即回退 |

## 完成标志

- 重构前后测试全绿，行为基线无破坏
- 每次提交只含单一重构手法，含 change ID
- 异味已消除/收窄，且无 scope creep

## 与相邻技能

| 场景 | 归属 |
|------|------|
| 功能编码 | `/coding-skill` |
| 补特征测试保护 | `/unit-test-write` |
| 评审重构结果 | `/expert-reviewer` |
| 重构执行 | **本技能**（`/harness-refactor`） |