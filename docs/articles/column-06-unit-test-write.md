...
### 1.1 从 tayama-trip-plan 的"测试焦虑"说起

在 tayama-trip-plan 的实践中，测试有一个"焦虑"：**不知道测够了没有。**
...
在 AI 应用（如 tayama-trip-plan）中，降级逻辑非常重要：当 LLM 超时、API 不可用、token 超限时，系统要"优雅降级"——返回离线估算、触发人工干预（HITL）、走缓存等。
...
在 tayama-trip-plan 的实践中，团队从"写完代码再补测试"到"先写测试再写代码"，是一种文化转变。TDD 不是"测试先行"，而是"思考先行"——先想清楚"什么行为是对的"，再动手写代码。
