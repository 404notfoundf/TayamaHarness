...
更糟糕的是，当团队从 Java 扩展到 Python、Go、Rust、PHP、Frontend 时，复制粘贴彻底不可维护了。**复制粘贴只适用于"同质化"的项目，而现实是"异质化"的。**
...
- 6 种语言的检测规则（每种语言 10-34 条，PHP 最多 34 条）
...
这一层的检测规则最为丰富——仅 Java 就有 19 种框架检测规则，Python 有 23 种，Go 有 22 种，Frontend 有 10 种，Rust 有 20 种，PHP 有 34 种。
...
`apply-harness` 目前定义了 111 个框架差异块，覆盖了 6 种语言的 60+ 个框架。这个数字不是"越多越好"，而是"越精确越好"。
...
`apply-harness` 的 130KB 不是"代码臃肿"，而是"覆盖度"的体现。它覆盖了 6 种语言、60+ 个框架、111 个差异块、10 个核心模板——所有这些都在一个原子操作中完成。
